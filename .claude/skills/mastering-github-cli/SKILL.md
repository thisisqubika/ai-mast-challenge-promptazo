...
description: Use for ANY GitHub Actions query - workflow logs, action outputs, CI logs, job logs, step output, workflow status, action failures, workflow runs, CI failures, build logs, test logs, deployment logs, action results. Also handles GitHub repository operations via gh CLI - PRs, issues, artifacts, workflow triggers. ALWAYS use this skill when user asks about GitHub Actions, workflows, CI/CD, action logs, workflow output, gh commands, checking workflows, viewing runs, monitoring CI, or any GitHub automation.
